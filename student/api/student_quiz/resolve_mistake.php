...
if (!$data || !isset($data['mistake_id'])) {
    echo json_encode(['status' => 'false', 'message' => 'Invalid data']);
    exit();
}

$mistake_id = intval($data['mistake_id']);

// Delete the mistake record
$query = "DELETE FROM user_mistakes WHERE id = ? AND user_id = ?";
...
$stmt->bind_param("ii", $mistake_id, $user_id);

if ($stmt->execute()) {
    echo json_encode(['status' => 'true', 'message' => 'Mistake resolved']);
} else {
